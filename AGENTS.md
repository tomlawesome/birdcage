...
the mockingbird image (#95). That last gate currently fails: `hpfeeds`
(pulled in by `opencanary`) is GPL-3.0, and `setuptools`/`ordereddict`
declare no licence at all. Fixing that is the owner's call, not this
gate's.
