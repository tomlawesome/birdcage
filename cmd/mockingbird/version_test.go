...
// The release job compares `mockingbird version` with the tag it built
// from, so what matters is that the command prints the stamped variable
// and nothing else: a hardcoded string or extra output would both pass a
// naive "it printed something" check and fail the comparison. Mirrors
// cmd/birdcage/version_test.go's own TestRunVersionPrintsTheStampedValue.
