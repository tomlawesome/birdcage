...
func TestEnrolRunCommandEscapesOperatorSuppliedValues(t *testing.T) {
	var out strings.Builder
	if err := printEnrolRunCommand(&out, "203.0.113.10\x1b[31m", "8444", "deadbeef", "cafebabe", "image\x07name", true, "old-fs-01\x1b[32m", "linux\x07"); err != nil {
		t.Fatalf("printEnrolRunCommand: %v", err)
	}
	got := out.String()
...
func TestEnrolRunCommandCarriesThePoisonerSettings(t *testing.T) {
	var out strings.Builder
	if err := printEnrolRunCommand(&out, "203.0.113.10", "8444", "deadbeef", "cafebabe", "mockingbird:latest", false, "old-fs-01,printer-7", "linux"); err != nil {
		t.Fatalf("printEnrolRunCommand: %v", err)
	}
	got := out.String()
	for _, want := range []string{
