...
func TestSelect(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "the documented default address with no certificate is a refusal",
			addr:    ":8080",
			wantErr: true,
		},
		{
			name:    "0.0.0.0 with no certificate is a refusal",
			addr:    "0.0.0.0:8080",
			wantErr: true,
		},
		{
			name:    "a real hostname with no certificate is a refusal",
			addr:    "dashboard.example.com:8080",
			wantErr: true,
		},
		{
			name:    "a LAN IP with no certificate is a refusal",
			addr:    "192.168.1.5:8080",
			wantErr: true,
		},
		{
			name:    "an address with no port at all is a refusal",
			addr:    "127.0.0.1",
			wantErr: true,
		},
		{
			name:    "empty address with no certificate is a refusal",
			addr:    "",
			wantErr: true,
		},
	}
